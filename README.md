# JobSearchUs — Remote Job Listings

**132 remote jobs** across 19 categories — updated 2026-05-15

Fresh remote jobs pulled straight from company career pages — not recycled from job boards.
Updated 5 times a day across 1,500+ companies worldwide.

Full job board with filters: **[jobsearchus.com](https://www.jobsearchus.com)**

---

## How it works

1. We track 1,500+ company career pages directly
2. New job postings are detected within hours of going live
3. Each listing is verified, categorized, and added here — no duplicates, no spam

---

## Browse by Category

| Category | Open Roles | Link |
| --- | --- | --- |
| Work From Home Jobs | 132 | [View →](jobs/work-from-home-jobs.md) |
| $100K+ Remote Jobs | 27 | [View →](jobs/100k-plus-jobs.md) |
| Entry Level Remote Jobs | 6 | [View →](jobs/entry-level-jobs.md) |
| Software Engineer Jobs | 24 | [View →](jobs/software-engineer-jobs.md) |
| Backend Jobs | 5 | [View →](jobs/backend-jobs.md) |
| Fullstack Jobs | 1 | [View →](jobs/fullstack-jobs.md) |
| Developer Jobs | 4 | [View →](jobs/developer-jobs.md) |
| DevOps Jobs | 1 | [View →](jobs/devops-jobs.md) |
| Site Reliability Engineer Jobs | 2 | [View →](jobs/site-reliability-engineer-jobs.md) |
| Data Science Jobs | 4 | [View →](jobs/data-science-jobs.md) |
| Product Manager Jobs | 1 | [View →](jobs/product-manager-jobs.md) |
| UI/UX Jobs | 1 | [View →](jobs/ui-ux-jobs.md) |
| Marketing Jobs | 10 | [View →](jobs/marketing-jobs.md) |
| Sales Jobs | 20 | [View →](jobs/sales-jobs.md) |
| Customer Support Jobs | 6 | [View →](jobs/customer-support-jobs.md) |
| Finance Jobs | 6 | [View →](jobs/finance-jobs.md) |
| HR Jobs | 2 | [View →](jobs/hr-jobs.md) |
| Legal Jobs | 1 | [View →](jobs/legal-jobs.md) |
| Operations Jobs | 12 | [View →](jobs/operations-jobs.md) |
| Security Jobs | 4 | [View →](jobs/security-jobs.md) |
| Creative Jobs | 6 | [View →](jobs/creative-jobs.md) |
| Admin Jobs | 1 | [View →](jobs/admin-jobs.md) |

---

## Follow us

Stay updated when new jobs drop:

- **Website:** [jobsearchus.com](https://www.jobsearchus.com)
- **X (Twitter):** [@JobSearchUss](https://x.com/JobSearchUss)
- **LinkedIn:** [jobsearchus](https://linkedin.com/company/jobsearchus)
- **Instagram:** [@jobsearchus_](https://instagram.com/jobsearchus_)
- **Bluesky:** [jobsearchus.bsky.social](https://bsky.app/profile/jobsearchus.bsky.social)

---

*Updated 2026-05-15 · Jobs sourced directly from company career pages · [jobsearchus.com](https://www.jobsearchus.com)*
